...
```{r}
se.filtered.npm_melt = melt(se.filtered) %>%
...
```{r}
npm.neg = se.filtered[, 2:869]
npm.pos = se.filtered[, 870:944]
```
...
se.filtered.wilcox = row_wilcoxon_twosample(npm.neg, npm.pos, exact = T, alternative = "two.sided", correct = F)
```
...
```{r}
se.npm.wilcox.sig_melt = melt(se.npm.wilcox.sig.norm) %>% #normalized
  mutate(group = ifelse(grepl("NEG", variable), "NPM.NEG", "NPM.POS"))
```
